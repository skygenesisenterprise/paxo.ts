...
export interface GuiModule {
  // Window management
  window(options?: { title?: string; size?: Size; position?: Point }): Window;

  // Drawing
  canvas(size: Size): Canvas;

  // Widgets
  button(options: { text: string; position: Point; size?: Size; onClick?: () => void }): Button;
  label(options: { text: string; position: Point; color?: Color }): Label;
  input(options: { position: Point; size?: Size; placeholder?: string }): Input;
  list(options: { position: Point; size: Size; items: string[] }): List;
...
// Base App class for structured development
